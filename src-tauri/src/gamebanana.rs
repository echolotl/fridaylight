...
pub async fn fetch_gamebanana_mods(
  ...
  let (mods_data, total_count) = match mods_array {
    Ok(array) => {
      debug!(
        "Successfully parsed API response as array of {} mods",
        array.len()
      );
    ...
    Err(_) => {
      ...
      debug!(
        "Failed to parse as array, trying to parse as object with _aRecords"
      );
      ...
      match api_data {
        Ok(data) => {
          ...
          if
            let Some(records) = data.get("_aRecords").and_then(|r| r.as_array())
          {
  ...
  for record in &mods_data {
    ...
    mods.push(GameBananaMod {
      ...
      image_url: record
        ...
        .unwrap_or("")
        .to_string(),
      initial_visibility: record
        .get("_sInitialVisibility")
        .and_then(|v| v.as_str())
        .unwrap_or("")
        .to_string(),
      period: record
      ...
      view_count: record
        ...
        .and_then(|v| v.as_i64())
        .unwrap_or(0),
      is_owned_by_accessor: record
        .get("_bIsOwnedByAccessor")
        .and_then(|v| v.as_bool())
        .unwrap_or(false),
      was_featured: record
        .get("_bWasFeatured")
...
pub async fn get_mod_version(
  ...
  match mod_info.get("_sVersion") {
    Some(version) => {
      if let Some(version_str) = version.as_str() {
        Ok(version_str.to_string())
      } else {
        Err("Version is not a string".to_string())
      }
    }
    None => Err("Version not found in mod info".to_string()),
  }
}
...
pub async fn get_download_url(
  ...
  let download_page: serde_json::Value = match
    download_page_response.json().await
  {
    Ok(data) => data,
    Err(e) => {
...
pub fn extract_banner_url(
  mod_info: &serde_json::Value,
  mod_id: i64,
  model_type: &str
) -> Option<String> {
  mod_info
    ...
    .or_else(|| {
      ...
      Some(
        format!(
          "https://gamebanana.com/{}/embeddables/{}",
