    ...
    if let Ok(icon_dir) = ico::IconDir::read(file) {
      if !icon_dir.entries().is_empty() {
        ...
        if let Ok(image) = entry.decode() {
          ...
          let rgba_image = ImageBuffer::<Rgba<u8>, _>
            ::from_raw(width, height, rgba_data.to_vec())
            .unwrap_or_else(|| {
              eprintln!(
                "Warning: Failed to create ImageBuffer from icon data. Using empty buffer."
              );
      ...
      if icon_info_result != 0 {
        ...
        if !h_dc.is_null() {
          ...
          bitmap_info.bmiHeader.biSize =
            mem::size_of::<BITMAPINFOHEADER>() as u32;
...
pub fn check_for_custom_images(
  mod_folder: &Path
        ...
        match serde_json::from_str::<serde_json::Value>(&content) {
          Ok(json) => {
            ...
            match metadata_version {
              None => {
                warn!(
                  "metadata.json is missing required metadata_version field"
                );
                return None;
              }
              Some(version) if version < MIN_METADATA_VERSION => {
                warn!(
                  "metadata.json version {} is too old. Minimum supported version is {}",
                  version,
...
pub fn save_metadata_json(
  ...
  if metadata_path.exists() {
    info!(
      "metadata.json already exists at: {}, skipping creation",
  ...
  match fs::write(&metadata_path, json_string) {
    Ok(_) => {
      debug!(
        "Successfully saved metadata.json to: {}",
        metadata_path.display()
      );
      Ok(())
    }
...
pub fn create_mod_info(path: &str) -> Result<ModInfo, String> {
  ...
  let contributors = match &metadata {
    Some(json) => {
      if
        let Some(contributors_array) = json
          .get("contributors")
          .and_then(|v| v.as_array())
      {
        ...
        for group_object in contributors_array {
          if let Some(group_obj) = group_object.as_object() {
            // Get the group name
            if
              let Some(group_name) = group_obj
                .get("group")
                .and_then(|v| v.as_str())
            {
              // Get the members array
              if
                let Some(members_array) = group_obj
                  .get("members")
                  .and_then(|v| v.as_array())
              {
        ...
        if !contributor_groups.is_empty() {
          debug!(
            "Parsed {} contributor groups with total of {} members",
  ...
  let description = match &metadata {
    Some(json) => {
      if
        let Some(desc_value) = json.get("description").and_then(|v| v.as_str())
      {
  ...
  let engine = match &metadata {
    Some(json) => {
      if let Some(engine_json) = json.get("engine") {
        ...
        let engine_type = engine_json
          .get("engine_type")
          .and_then(|v| v.as_str())
          .map(|s| s.to_string());

        // Extract engine_name from engine object
        let engine_name = engine_json
          .get("engine_name")
          .and_then(|v| v.as_str())
          .map(|s| s.to_string());
        // Extract engine_icon from engine object
        let engine_icon = engine_json
          .get("engine_icon")
          .and_then(|v| v.as_str())
          .and_then(|icon_path| {
        ...
        let mods_folder = engine_json
          .get("mods_folder")
          .and_then(|v| v.as_bool());

        // Extract mods_folder_path
        let mods_folder_path = engine_json
          .get("mods_folder_path")
          .and_then(|v| v.as_str())
          .map(|s| s.to_string());
...
pub fn is_valid_fnf_mod(path: &Path) -> bool {
  ...
  debug!(
    "Validity check for {}: assets={}, manifest={}, executable={}, valid={}",
    path.display(),
    has_assets,
    has_manifest,
    has_executable,
