...
class DownloadStateManager {
  ...
  public updateDownloadProgress(
    progress: Partial<DownloadProgress> & { id: string }
  ) {
    ...
    if (this.downloadingMods[progress.id]) {
      ...
      console.warn(
        `Could not update download progress: ID ${progress.id} not found in downloadingMods`
      )
    }
  }
...
export const updateDownloadProgress =
  downloadState.updateDownloadProgress.bind(downloadState)
export const completeDownload =
  downloadState.completeDownload.bind(downloadState)
